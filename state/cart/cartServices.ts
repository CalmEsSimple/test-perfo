...
export const getCart = async () => {
  ...
  const response = await axios.get(`${process.env.NEXT_PUBLIC_ENDPOINT_URL_BASE}/cart/get_cart.php`, requestConfig);
...
export const getVariation = async (id: string) => {
  ...
  const response = await axios.get(`${process.env.NEXT_PUBLIC_ENDPOINT_URL_BASE}/products/get_variation_by_id.php?id=${id}`, requestConfig);
...
export const getCartRelatedProducts = async (ids: string[]) => {
  ...
  const response = await axios.get(
    `${process.env.NEXT_PUBLIC_ENDPOINT_URL_BASE}/products/get_cross_selling_cart_products.php?ids=${ids.join(";")}`,
    requestConfig
  );
  return await response.data;
}