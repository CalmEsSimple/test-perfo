...
export const onAddItemToCart = (id: number, quantity: number) => {
  return async (dispatch: any) => {
    ...
    if (response.dataCart.status == 200) {
      ...
      dispatch(onAddItemToCartFailed());
      dispatch(onAddToCartLoadingFinished())
    }
  };
};
...
const onAddItemToCartFailed = () => ({
  type: ON_ADD_ITEM_TO_CART_FAILED
});

export const onRemoveItemFromCart = (key: string) => {
  return async (dispatch: any) => {
...
export const onUpdateItemFromCart = (key: string, quantity: number) => {

  return async (dispatch: any) => {

    dispatch(onUpdateToCartLoadingStart())
    const response = await updateItem(key, quantity);
